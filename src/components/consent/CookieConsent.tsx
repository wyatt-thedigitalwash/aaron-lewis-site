...
import { useSplash } from "@/components/site/SplashGate";
...
export default function CookieConsent() {
  // A visitor has "entered" the site only once they explicitly click Enter Site
  // (SplashGate.entered), NOT merely when the splash is hidden -- a visitor
  // deep-linking to /legal has the splash hidden but has not entered. Gating on
  // `entered` keeps the banner from stacking on the splash and from popping up
  // on a legal page someone is reading before entering.
  ...
  // the site -- otherwise it (and the TermsGate popup that follows it) pile
  // up on top of someone who's just detoured to read the Terms via the splash.
  useEffect(() => {
                  ...
                  {cat.locked ? (
                    ...
                    <button
                      ...
                      className={`mt-1 flex h-6 w-11 shrink-0 items-center rounded-full px-0.5 transition-colors ${
                        draft[cat.key] ? "bg-accent justify-end" : "bg-text-body/25 justify-start"
                      }`}
                    >
